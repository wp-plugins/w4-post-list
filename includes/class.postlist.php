...
class W4_Post_list {
	...
	function template_category_posts(){
		if( $this->show_post_list ){
			...
			if( 'yes' == $this->list_effect )
				$postlist = "<div class='term_posts term_posts_". $this->current .'_'. $this->category_obj->term_id ."'>" . $postlist . "</div>";

			elseif( 'extended' == $this->list_effect )
				$postlist = "<div class='term_posts term_posts_". $this->current . '_'. $this->category_obj->term_id ."' style='display:none;'>" . $postlist . "</div>";
	...
	function template_category_title(){
		$title = "View all posts from";
		if( $this->category_obj->count < 1 ){
			$this->category_link_class = "no_posts";
			$title = "No posts in";
		}
		else{
			$this->category_link_class = "have_posts";
		}

		if( $this->list_has_effect ){
			if( $this->show_post_list ){
				$this->category_link_class .= " available_posts";
...
#		if( $this->category_obj->count < 1 )
#			return sprintf( '<a class="%1$s" href="%2$s" alt="%4$s" title="'. __( 'No posts in %3$s', 'w4-post-list' ) .'">%3$s</a>', $this->category_link_class, '[cat_link]', '[cat_name]', $this->category_obj->term_id );

#		else
			return sprintf( '<a class="%1$s" href="%2$s" ref="term_posts_%5$s_%4$s" alt="%4$s" title="'. __( $title .' %3$s', 'w4-post-list' ) .'">%3$s</a>',
		$this->category_link_class, '[cat_link]', '[cat_name]', $this->category_obj->term_id, $this->current );
	}

	function template_category_count(){
		if( $this->category_obj->count > 0 )
	...
	function template_title(){
		return sprintf( '<a class="post_title w4pl_post_title" href="%1$s" title="View %2$s">%2$s</a>', '[post_permalink]', '[post_title]' );
	}
	...
	function template_more(){
		...
		return sprintf( '<a class="read_more" href="%1$s" title="Cotinue reading %2$s">%3$s</a>', '[post_permalink]', '[post_title]',  $read_more );
	}
	...
	function template_image(){
		...
		$attr = array(
			...
			'alt'				=> $alt,
			'title'				=> $title,
			'class'				=> "attachment-{$this->image_size} {$this->image_source} w4pl_post_thumb",
