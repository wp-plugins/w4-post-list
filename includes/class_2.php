...
class W4_Post_list {
	...
	function generate_category_list(){
		...
		foreach( (array)$this->categories as $cat_id => $cat_option){
			...
			// Gather the category info..
			$category_template = $this->category_template( $cat_id);

			if( !$category_template )
				continue;

			if( 'yes' == $this->list_effect && $this->show_post_list )
				$this->category_class .= ' open';

			if( 'extended' == $this->list_effect && $this->show_post_list )
				$this->category_class .= ' close';

			// Category contaner class
			$category_li_class = !empty( $this->category_class) ? " class='$this->category_class'" : '';
			
			$output .= "<li {$category_li_class}>";
				...
				if( $this->show_post_list ){
					...
					if( 'yes' == $this->list_effect )
						$postlist = "<div class='list_has_js_effect'>" . $postlist . "</div>";
					
					elseif( 'extended' == $this->list_effect )
						$postlist = "<div class='list_has_js_effect' style='display:none;'>" . $postlist . "</div>";
	...
	function category_template( $cat_id ){
		...
		//	'post__not_in' 		=> $this->categories[$this->category_obj->term_id]['posts_not_in'],
			...
			'order' 			=> $post_order['order'],
			'orderby' 			=> $post_order['orderby'],
			'posts_per_page'	=> intval( $this->categories[$this->category_obj->term_id]["max"]) > 0 ? 
			intval( $this->categories[$this->category_obj->term_id]["max"]) : '-1');

		$this->query['showposts'] = $this->query['posts_per_page'];

		$this->categories[$this->category_obj->term_id]['class'] = "";

		//Only build the jquery show/hide if post and category list type selected
		
		if( $this->list_has_effect && $this->show_post_list )
		...
		$output = $this->category_title();

		if( $this->show_count )
			$output .= $this->category_count();
	...
	function publish(){
		return sprintf( ' <abbr class="published" title="%2$s"><strong>' . __("Published:", "w4-post-list").'</strong> %1$s</abbr>',	get_the_time('j-m-Y'), get_the_time('g:i a'));
	}
	...
	function category_title(){
		...
		if( $this->list_has_effect ):
			$this->category_class = "list_effect";
			$title = "<span class=\"marker\"></span>$title";
		endif;

		return sprintf( '<a class="%1$s" href="%2$s" title="'. __( 'View all in %3$s', 'w4-post-list' ) .'">%4$s</a>',
		$this->category_link_class, get_category_link( $this->category_obj->term_id ), $this->category_obj->name, $title );
	}
}
